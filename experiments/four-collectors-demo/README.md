...
- **curl** → the agent's **first attempt failed** (zsh globbed the unquoted `?` in the URL) and was captured as `PostToolUseFailure`; the retry succeeded (HTTP 200). Network collector caught the flow `192.0.2.10:63997 -> 203.0.113.220:443 (ESTABLISHED)` (the Cloudflare speed-test endpoint; both IPs shown as documentation-range placeholders); process collector attributed it: `curl` (pid 1417) ← `zsh` (1414) ← the Claude node process (220).
...
- **No packet contents.** We record that curl connected to `203.0.113.220:443`, not the bytes sent (TLS anyway). "Data left the machine" must be phrased as "an outbound connection/transfer-shaped command ran to X."
